...
from untils import Metrics,label_text,Metrics_e


def ner_eval(model: BertNer, data_loader: DataLoader):
    allreal=[]
    allpre=[]
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm(data_loader):
            ...
            for pre, real in zip(bacth_output, true_tags):
                ...
                allpre.append(label_text(pre))
                allreal.append(label_text(real))
    me=Metrics(allpre,allreal)
    me.report_scores()
    Metrics_e(allpre,allreal)
...
def ner_train(model: BertNer, train_data_loader, epoch: int, learning_rate: float = 1e-5):
    ...
    for batch_data in tqdm(train_data_loader):
        batch_sentence_id, batch_sentence_mask, batch_label = batch_data
        batch_masks = batch_sentence_id.gt(0)
        loss = model(batch_sentence_id=batch_sentence_id, batch_mask=batch_masks, batch_sentence_mask=batch_sentence_mask,
